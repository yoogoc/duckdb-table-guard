...
struct AllowEntry {
    ...
    string str() const {
        auto fmt = [](const string &s) { return s.empty() ? "*" : s; };
...
static bool ExtractTableIdentifier(const LogicalGet &get,
                                   string &out_catalog,
                                   string &out_schema,
                                   string &out_table) {
...
static const vector<string> METADATA_FUNCTIONS = {
...
static MetadataColumnPositions FindMetadataColumns(LogicalGet &get) {
    ...
    if (db_orig == DConstants::INVALID_INDEX ||
        schema_orig == DConstants::INVALID_INDEX ||
...
static unique_ptr<Expression> MakeEqualityFilter(idx_t table_index, idx_t col_pos, const string &value) {
    auto col_ref = make_uniq<BoundColumnRefExpression>(
        LogicalType::VARCHAR,
        ColumnBinding(table_index, col_pos)
    );
...
static unique_ptr<Expression> BuildAllowlistFilter(
    const TableGuardState &guard,
    idx_t table_index,
    ...
    if (op.type == LogicalOperatorType::LOGICAL_GET) {
        ...
        if (ExtractTableIdentifier(get, catalog, schema, table)) {
            ...
            if (!ok) {
                throw PermissionException(
                    "TableGuard: \"%s.%s.%s\" is not in the allowlist.\n"
                    "  Allowed: %s",
        ...
        for (const auto &meta_func : METADATA_FUNCTIONS) {
            if (get.function.name == meta_func) {
                ...
                if (!cols.found) {
                    throw PermissionException(
                        "TableGuard: metadata query \"%s\" blocked "
...
struct TableGuardOptimizer : OptimizerExtension {
    ...
    static void Run(OptimizerExtensionInput &input,
...
static void PragmaAllow(ClientContext &ctx, const FunctionParameters &params) {
    ...
    if (guard->configured) {
        throw PermissionException(
            "TableGuard: allowlist has already been configured for this connection "
    ...
    if (raw.empty()) {
        throw InvalidInputException(
            "TableGuard: table_guard_allow requires a non-empty argument.\n"
    ...
    if (guard->allowed.empty()) {
        throw InvalidInputException(
            "TableGuard: no valid entries found in the allowlist string: '%s'", raw
        );
    }
...
static void PragmaStatus(ClientContext &ctx, const FunctionParameters &) {
    ...
    Printer::PrintF(
        "TableGuard: enabled=%s, configured=%s, allowlist=%s",
        guard->enabled    ? "true" : "false",
        guard->configured ? "true" : "false",
        guard->AllowedStr()
    );
}
...
static void LoadInternal(ExtensionLoader &loader) {
    ...
    loader.RegisterFunction(
        PragmaFunction::PragmaStatement("table_guard_enable", PragmaEnable)
    );

    loader.RegisterFunction(
        PragmaFunction::PragmaStatement("table_guard_disable", PragmaDisable)
    );

    loader.RegisterFunction(
        PragmaFunction::PragmaStatement("table_guard_status", PragmaStatus)
    );
}
